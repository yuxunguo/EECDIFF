...
'''
print(np.array([1.,1.]) @ evolop(J, NF, P, 10, 2, nloop))

print(Gamma_Evo(np.array([0.754,0.824]),10,1))

print(Gamma_tilde_Perturbative_Evo(np.array([0.754,0.824]),10,0.1,1))
'''
...
def GammaImprov(theta: float, Q: float, Gamma_Init: np.array, bmax: float, Gnonpert: float, nlooplog: int, MU0: float, cimpoff = False):
...
def EEC_Compute_Aux(args):
    ...
    # Central values
    f_central = dEEC(theta, Q, gammainit, bmax, gq, gg, fq, fg, nlooplog)
    fz_central = 2 * theta / np.sin(theta) * f_central

    # Scan ranges
    gq_range = np.linspace(gq / factor, gq * factor, nscan)
    gg_range = np.linspace(gg / factor, gg * factor, nscan)
    bmax_range = np.linspace(bmax - deltab, bmax + deltab, nscanb)
    
    # Prepare storage
    f_vals = np.empty((nscan, nscan, nscanb))
    fz_vals = np.empty_like(f_vals)

    # Triple scan: (gq, gg, bmax)
    for i, gq_var in enumerate(gq_range):
        for j, gg_var in enumerate(gg_range):
            for k, bmax_var in enumerate(bmax_range):
                f_tmp = dEEC(theta, Q, gammainit, bmax_var, gq_var, gg_var, fq, fg, nlooplog)
                f_vals[i, j, k] = f_tmp
                fz_vals[i, j, k] = 2 * theta / np.sin(theta) * f_tmp

    # Compute envelope
    fmin, fmax = np.min(f_vals), np.max(f_vals)
    fzmin, fzmax = np.min(fz_vals), np.max(fz_vals)

    return (theta, Q, f_central, z, fz_central, fmin, fmax, fzmin, fzmax)
    '''
...
def dEEC_Res_cal_plt(theta_lst, Q_lst, gammainit,bmax,gq,gg,fq,fg,nlooplog):
    ...
    df = pd.DataFrame(EEC, columns=["theta", "Q", "dEEC","z","dEECz", "dEECmin", "dEECmax", "dEECzmin", "dEECzmax"])
...
def dEEC_cal_plt(zlst, Q_lst):
    ...
    # TMD
    #'''
    ax.plot(df_Resum_Q1["z"],scales[0]* df_Resum_Q1["dEECz"], marker='o', linestyle='', linewidth=0.5, color=colors[0], markersize = 3, markerfacecolor='none')
    ax.plot(df_Resum_Q2["z"],scales[1]* df_Resum_Q2["dEECz"], marker='o', linestyle='', linewidth=0.5, color=colors[1], markersize = 3, markerfacecolor='none')
    ax.plot(df_Resum_Q3["z"],scales[2]* df_Resum_Q3["dEECz"], marker='o', linestyle='', linewidth=0.5, color=colors[2], markersize = 3, markerfacecolor='none')
    #'''
    
    ax.fill_between(df_Resum_Q1["z"], scales[0] * df_Resum_Q1["dEECzmin"], scales[0] * df_Resum_Q1["dEECzmax"], color=colors[0], alpha=0.4, linewidth=0.5)
    ax.fill_between(df_Resum_Q2["z"], scales[1] * df_Resum_Q2["dEECzmin"], scales[1] * df_Resum_Q2["dEECzmax"], color=colors[1], alpha=0.4, linewidth=0.5)
    ax.fill_between(df_Resum_Q3["z"], scales[2] * df_Resum_Q3["dEECzmin"], scales[2] * df_Resum_Q3["dEECzmax"], color=colors[2], alpha=0.4, linewidth=0.5)

    def grouplegend(colors, linesty, markersty):
        
        group = (
        (Line2D([0], [0], color=colors[0], linestyle=linesty, linewidth=1), Line2D([0], [0], color=colors[0], marker=markersty, linestyle='None', markersize = 3, markerfacecolor='none')),
        (Line2D([0], [0], color=colors[1], linestyle=linesty, linewidth=1), Line2D([0], [0], color=colors[1], marker=markersty, linestyle='None', markersize = 3, markerfacecolor='none')),
        (Line2D([0], [0], color=colors[2], linestyle=linesty, linewidth=1), Line2D([0], [0], color=colors[2], marker=markersty, linestyle='None', markersize = 3, markerfacecolor='none'))
        )
        return group
    ...
    square_handles  = [
    SmallSquare(facecolor=colors[0], edgecolor='white', label=fr'$\bar E$ = {Q_lst[0]} GeV'+rf'$ (\times 10^{3})$',size=4),
    SmallSquare(facecolor=colors[1], edgecolor='white', label=fr'$\bar E$ = {Q_lst[1]} GeV',size=4),
    SmallSquare(facecolor=colors[2], edgecolor='white', label=fr'$\bar E$ = {Q_lst[2]} GeV'+rf'$ (\times 10^{{{-3}}})$',size=4),
    ]
    ...
    ax.set_ylabel(r"$\mathrm{d}\Sigma/(\sigma_0 \mathrm{d}z)$",fontsize=13)
    ax.set_title(r"$\mathrm{d}\Sigma/(\sigma_0 \mathrm{d}z)$ vs z compared with the collinear ones",fontsize=14)
    ax.grid(True)
    ax.set_xscale("log")
    ax.set_yscale("log")
...
def Gamma_qT_Q_CalcCsv(qT, Qlst):
...
def Gamma_theta_Q_CalcCsv(theta, Qlst):
    ...
    def compute_gamma_curve(Q):
        ...
        return theta, np.array(Gq_vals), np.array(Gg_vals)
    ...
    for Q in Qlst:
        thetaQ, Gq, Gg = compute_gamma_curve(Q)
        for theta_val, gq_val, gg_val in zip(thetaQ, Gq, Gg):
            rows.append({
                "Q": Q,
                "qT": np.sin(theta_val) * Q,
                "gamma_q": gq_val,
                "gamma_g": gg_val
            })
...
def dEECimprovSim_helper(args):
    ...
    return dEECimprovSim(theta, Q, Gammainit, bmax, Gnonpert, nlooplog, MU0)

def dEECimprovNLO_helper(args):
    qT, Q = args
    theta = qT/Q

    return dEECimprovNLO(theta, Q, Gammainit, bmax, Gnonpert, nlooplog, MU0)

def dEEC_qT_Q_Cal(qT_list, Q_list):
    
    all_args = list(product(qT_list, Q_list))
    
    with Pool() as pool:
        sim_results = pool.map(dEECimprovSim_helper, all_args)
        nlo_results = pool.map(dEECimprovNLO_helper, all_args)

    # Create DataFrame
    df = pd.DataFrame({
        "qT": [arg[0] for arg in all_args],
        "Q": [arg[1] for arg in all_args],
        "dEECimprovSim": sim_results,
        "dEECimprovNLO": nlo_results
    })
