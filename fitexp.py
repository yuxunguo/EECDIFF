    ...
    for fig_idx in range(0, n_plots, n_per_figure):
        ...
        for row in range(nrows_eff):
            for col in range(ncols):
                ...
                ax.plot(theta, y_f, '*', markersize=8, label=datalabel,
                        alpha=1.0, color=c_data)

                ax.plot(theta, y_pred, label="Imprv. LLA+NLO",
                        color=c_pred, linestyle='-', linewidth=1.5)

                ax.plot(theta, y_predimp, label="Imprv. LLA",
                        color=c_predimp, linestyle='--', linewidth=1.5)

                # Truncate divergent NNLL curve before plotting
                mask = (y_predNNLL > ymin_plot) & (y_predNNLL < ymax_plot)

                ax.plot(theta[mask], y_predNNLL[mask],
                        linestyle='-.', label="Collinear NNLL",
                        color=c_nnll, linewidth=1.5)
        ...
        for row in range(nrows_eff):
            for col in range(ncols):
                ...
                ax.errorbar(theta, y_f, yerr=y_err, fmt='o', capsize=3, markersize=3,
                            label=datalabel, alpha=1, color=c_data,zorder=1)

                ax.plot(theta, y_pred, label="Imprv. LLA+NLO",
                        color=c_pred, linestyle='-', linewidth=1.5, zorder=4)

                ax.plot(theta, y_predimp, label="Imprv. LLA",
                        color=c_predimp, linestyle='--', linewidth=1.5, zorder=3)
                ...
                ax.plot(theta[mask], y_predNNLL[mask],
                        linestyle='-.', label="Collinear NNLL",
                        color=c_nnll, linewidth=1.5, zorder=2)
